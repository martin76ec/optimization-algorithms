...
def particle_swarm_opt(f, p_num, n, bounds, max_epochs=100, alph1=1, alph2=1):
    ...
    while curr_epoch < max_epochs:
        for particle in particles:
            new_v = (
                particle.v
                + alph1 * phi1 * (particle.best_x - particle.x)
                + alph2 * phi2 * (gbest - particle.x)
            )

            new_x = particle.x + new_v
            current_score = f(new_x)

            if current_score < f(particle.best_x):
                particle.best_x = new_x
            if current_score < f(gbest):
                gbest = new_x

            particle.v = new_v
            particle.x = new_x

        g_best_score = f(gbest)
        scores.append(g_best_score)

        # print("Best fitness: ", g_best_score)
        curr_epoch += 1
    return gbest, scores[-1], scores
